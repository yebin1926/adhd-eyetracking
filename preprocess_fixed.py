...
def filter_valid_pi_fullnames(pi_mod, fullnames: List[str]) -> List[str]:
    """
    Remove non-PI rows (e.g., section headers like 'Eye Tracking') by validating
    with the PI extractor's split_fullname().
    """
    valid = []
    bad = []
    for name in fullnames:
        ...
        if not s:
            continue
        try:
            # PI module defines split_fullname(fullname) and raises if invalid
            pi_mod.split_fullname(s)
            valid.append(s)
        except Exception:
            bad.append(s)
    ...
    if len(valid) == 0:
        raise RuntimeError(
            "No valid PI fullnames found after filtering. "
            f"Examples of invalid entries: {bad[:10]}"
        )
    return valid
...
def main() -> None:
    ...
    fullnames_raw = load_pi_fullnames_from_excel(pi_excel_path, sheet_name=str(args.pi_sheet))
    fullnames = filter_valid_pi_fullnames(pi_mod, fullnames_raw)
    n_feat = len(fullnames)
    print(f"[PI] loaded {len(fullnames_raw)} entries, kept {len(fullnames)} valid PI fullnames")
